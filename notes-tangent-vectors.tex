...
\begin{document}
    ...
    \begin{proposition}\label{prop:curve-to-derivation}[Curves define derivations]
 ...
 %       If $L(f) \neq 0$, then $L^U (f\vert _U ) = L (f) \neq 0$. This means 
  %      \[ [ L \neq 0 ] \Rightarrow [L^U \neq 0]. \] 
    ...
    \begin{proof}[Proof of Theorem \ref{thm:tpm-definitions}] 
        ...
        \[      \partial_i \vert _p (f) : =  \partial _i ( f  \circ \varphi ^{-1}  ) (p)      \]
    ...
    \end{align*}
    This proves \eqref{eq:derivation-is-partials}. The fact that \eqref{eq:tpm-basis} is linearly independent is left as an exercise.
