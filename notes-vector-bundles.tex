...
\begin{example}
    The \emph{cotangent bundle} is the vector bundle $ \pi _{\ast} :  T^{\ast}M \to M$ with 
    \[ (T^{\ast}M)_x : =  T_x^{\ast}M    : = (T_xM)^{\ast}\text{  for all }x \in M. \]
    For any chart $(U, \varphi)$ of $M$, we can build a trivialization $ \hat{\varphi } : \pi_{\ast}^{-1}(U) \to U \times \mathbb{R}^n $ of $T^{\ast}M$ as follows:
